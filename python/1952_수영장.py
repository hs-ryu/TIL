...
# 아이디어1 : 상태 트리로 그려본다
#   n월에 1달 비용만 내고 n+1월을 고려하거나, 3개월 비용을 내고 n+3월을 고려
#   1일권과 1개월 권 비용 중 작은 쪽을 택하면....
#   n월 수영장을 이용하는 비욜, s : n-1월까지의 비용, d : 1일 비용, m : 1개월 비용, m3 : 3개월 비용
#   1달 이용권은 이용일 수 0에도 비용이 발생.
#   -> 1알 이용권 * 이용일 수에서 비용이 0으로 처리됨.
#   3달 이용권은 n월 이용일 수 0이어도 고려해야함.
#   11월이나 12월에도 3개월 이용권 구입을 고려해야 함.
#   (1개월 이용 가격 > 3개월 이용권 가격)

# 아이디어2 : i월까지 j이용권으로 수영장에 다니는 최소비용 d[i]를 구한다.
# (1) i-1월의 최소비용 + min(1일권*이용일, 1개월 권), i >=1
# (2) i-3월의 최소비용 + 3개월 권, i >= 3
