...
#내가 푼 방법
def DFS(level, s_taste, s_cal):
    print(selec)
    global highst_taste
    # 제한 조건 1. 조합한 칼로리의 합이 주어진 L보다 크다면 더이상 탐색할 필요 없다.
    if s_cal > L:
        return
    if s_taste > highst_taste:
        highst_taste = s_taste
    if level == N:
        return
    selec[level] = material[level]
    DFS(level + 1, s_taste + selec[level][0], s_cal + selec[level][1])
    selec[level] = 0
    DFS(level + 1, s_taste, s_cal)
