...
def solution(nums):
    ...
    # 몇개 뽑을건지
    cnt = len(nums)//2
    # nums 배열에서 cnt개 뽑아서 조합
    combi = list(combinations(nums, cnt))
    for johab in combi:
        # 조합을 셋으로 뭉쳐서, 길이 저장
        length = len(set(johab))
        # 제일 긴 길이만 출력
        if length > answer:
            answer = length
    return answer
...
# 내장 함수 안쓰고 풀기.
# 실패 (함수안에서 함수 호출........ 이거 해결되면 될듯한데...)

'''
def johab(level, start, answer, cnt, total_length, arr, nums):
    if level >= cnt:
        length_arr = len(set(arr))
        if answer < length_arr:
            answer = length_arr
        return answer
    for i in range(start, total_length-cnt+level+1):
        arr[level] = nums[i]
        johab(level+1, i+1, answer, cnt, total_length, arr, nums)
...
def solution(nums):
    ...
    cnt = total_length//2
    arr = [0] * cnt
    ans = johab(0,0, answer, cnt, total_length, arr, nums)
    return ans
'''