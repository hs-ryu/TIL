...
# 아이디어 : A,B음식의 재료를 정하는 방법
#     원소의 개수가 N//2 개인 부분집합
#     N개에서 N//2개를 고르는 조합
#     다음 두 경우 두 음식의 맛의 차이는 같음
#     1) A : 1,2,3  B : 4,5,6
#     2) A : 4,5,6  B : 1,2,3

# 참고) A음식의 재로가 1,2,4일때 시너지 S를 계산하는 방법:
#     S = S1,2 + S2,1 + S1,4 + S4,1 + S2,4 + S4,2

# n번 재료를 A 또는 B 그릇에 넣는다.
#     0 <= n < N
#     A와 B의 크기는 N//2
#     넣을 수 잇는 쪽에만 넣음.
# 모든 재료에 대해 결정한 경우
...
아이디어2 : n개의 비트로 이루어진 i에 대해, 0과 1의 개수가 같은 경우,
    0인 비트 번호는 A의 재료, 1인 비트 번호는 B의 재료가 된다.
    재료가 정해지면 시너지를 계산한다.
